...
import Swiper from 'swiper' //引入swiper包
...
//   },
  mounted() {
    // console.log('mounted加载完毕')
    //派发action。通过vue来发起ajax请求
    this.$store.dispatch('getBannerList')
    //在newswiper之前，需要在结构加载完毕之前，
    //利用定时器，来设置当前组件在数据加载完之后进行setTime(()=>{},200)
    /*
   setTimeout(()=>{
  ...
  */
  ...
  watch: {
    //用来监听getBannerList数据的变化，因为这条数据发生过变换，
    getBannerList: {
      handler() {
        //nextTick会在循环结束之后，延迟回调
        this.$nextTick(()=>{
